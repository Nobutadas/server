...
-- Weapon can not be broken during these actions.
...
local handleBreak = {
    ...
    [xi.mobFamily.LAMIAE] = function(mob)
        if mob:getAnimationSub() ~= 1 then
            mob:setAnimationSub(1)
        end
    end,

    [xi.mobFamily.TROLL] = function(mob)
        if mob:getAnimationSub() ~= 1 then
            mob:setAnimationSub(1)
        end
    end,

    [xi.mobFamily.MAMOOL_JA] = function(mob)
        if mob:getAnimationSub() ~= 1 then
            mob:setAnimationSub(1)
        end
    end,
    ...
    -- Relies on animation sub being set to 0.
    [xi.mobFamily.MARID] = function(mob)
    ...
    [xi.mobFamily.QUTRUB] = function(mob)
